                    ...
                    case ConsoleKey.S:
                        {
                            ...
                            switch (smsTextFormat)
                            {
                                case SmsTextFormat.PDU:
                                    IEnumerable<ModemResponse<SmsReference>> smsReferences = await modem.SendSmsInPduFormatAsync(phoneNumber, Encoding.UTF8.GetBytes(smsMessage), smsCodingScheme);
