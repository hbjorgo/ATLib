...
{
    public class AbbreviatedPhoneNumberTests
    {
        [Theory]
        ...
        {
            AbbreviatedPhoneNumber sut = new(number, npi);

            Assert.Equal(expectedNumber, sut.Number);
            Assert.Equal(TypeOfNumber.Abbreviated, sut.TypeOfNumber);
        ...
        {
            Assert.Throws<ArgumentException>(() => new AbbreviatedPhoneNumber(number, npi));
        }
    }
}
