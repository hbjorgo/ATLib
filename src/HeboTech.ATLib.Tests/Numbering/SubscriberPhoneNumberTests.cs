...
{
    public class SubscriberPhoneNumberTests
    {
        [Theory]
        ...
        {
            SubscriberPhoneNumber sut = new(number, npi);

            Assert.Equal(expectedNumber, sut.Number);
            Assert.Equal(TypeOfNumber.Subscriber, sut.TypeOfNumber);
        ...
        {
            Assert.Throws<ArgumentException>(() => new SubscriberPhoneNumber(number, npi));
        }
    }
}
