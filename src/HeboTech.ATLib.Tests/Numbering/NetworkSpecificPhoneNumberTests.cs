...
{
    public class NetworkSpecificPhoneNumberTests
    {
        [Theory]
        ...
        {
            NetworkSpecificPhoneNumber sut = new(number, npi);

            Assert.Equal(expectedNumber, sut.Number);
            Assert.Equal(TypeOfNumber.NetworkSpecific, sut.TypeOfNumber);
        ...
        {
            Assert.Throws<ArgumentException>(() => new NetworkSpecificPhoneNumber(number, npi));
        }
    }
}
